...
def test_draft_proposal_uses_current_video_duration_range() -> None:
    assert DraftProposal(duration=1).duration == 1
    assert DraftProposal(duration=15).duration == 15
    with pytest.raises(ValidationError):
        DraftProposal(duration=0)
    with pytest.raises(ValidationError):
        DraftProposal(duration=16)
