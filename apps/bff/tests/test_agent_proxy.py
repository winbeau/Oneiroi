...
@pytest.mark.asyncio
async def test_bff_explicitly_proxies_agent_routes_and_sse_headers(tmp_path: Path) -> None:
    payload = json.dumps(
    ...
    capability_file = tmp_path / "agent-capabilities.json"
    capability_file.write_text(
        AgentProbeRecord(
            endpointHash=provider_endpoint_hash("https://provider.example/v1"),
    ...
    gateway = create_gateway(
        GatewaySettings(
            ...
            agent_capability_file=capability_file,
        ),
