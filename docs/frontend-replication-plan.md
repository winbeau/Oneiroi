...
| 阶段 | 状态 | 已实现内容 |
| --- | --- | --- |
| P0 | 已完成 | 真实首尾帧模板、Composer、质量/比例/时长、完整任务状态和浏览器 mock |
| P1 | 已完成 | BFF 资产/会话/任务 API、SSE 阶段流、取消与参数下载；前端 API 优先、离线 mock 回退 |
| P2 | 已完成 | 首尾帧、强度、Seed、快速/HQ、720p/1080p、参数复用、资产上传/筛选/删除 |
| P3 | 已完成 | Agent Prompt 整理、内部灵感检索、模板套用和用户确认后提交 |
| P4 | 已完成 | 响应式会话栏、键盘/焦点/reduced-motion、桌面与移动 Playwright 核心路径 |
| Pi 部署 | 已完成 | 已部署到 `pi5`，由用户级 systemd 服务持续运行，监听 `0.0.0.0:4173`；工作区地址为 `http://192.168.3.250:4173` |

当前 BFF 的任务执行是用于前端闭环验收的内存模拟器；真实 H100 Runner 接入保持既定 `/v1/jobs/i2v` 与 SSE 契约，不需要改变页面组件。
