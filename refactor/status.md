...
- Pi 已完成一次整机 reboot；BFF/Web/Tunnel 自动恢复。重启发现的旧 preview 重建 `dist` 竞态已修复，LAN/loopback 两个旧 units 及 `lan-preview`/`video-in` drop-ins 已彻底删除，当前无 LAN 4173 listener。
...
2. 用户明确不再进行第二次 reboot；旧 units 删除与依赖图已实时验证，但不声称完成删除后的第二次冷启动复验。
3. H100 当前没有 Oneiroi Runner/LTX inference 进程；真实生成继续等待 gpu-server Runner，不扩展 Oneiroi 自有 scheduler。
